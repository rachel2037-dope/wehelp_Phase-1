...
function go(e) {
  e?.preventDefault();
  const input = document.getElementById('hotel-id');
  const raw = (input?.value || '').trim();
  if (!/^[1-9]\d*$/.test(raw)) { alert('請輸入正整數'); return; }
  console.log('有執行，導向 /hotel/' + raw);
  window.location.href = `/hotel/${raw}`;
}

// 點「取得」按鈕
document.addEventListener('click', (e) => {
  const btn = e.target.closest('#hotel-go');
  if (btn) go(e);
});

// 在輸入框按 Enter
document.addEventListener('keydown', (e) => {
  if (e.key === 'Enter' && e.target.closest('#hotel-id')) {
    go(e);
  }
});
});
